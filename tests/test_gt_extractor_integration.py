...
try:
    ...
    # Verify it's a real package, not a stub ModuleType
    _HAS_TRANSFORMERS = hasattr(_tf, "__file__") and _tf.__file__ is not None
except ImportError:
...
# Stub heavy optional deps if not installed
...
class TestGTExtractorIntegration:
    ...
    def test_output_shape_T4(self, extractor):
        """(1, 3, 4, 224, 224) → (1, 768, 2, 14, 14)"""
        video = torch.rand(1, 3, 4, 224, 224)
        feats = extractor(video)
        assert feats.shape == (1, 768, 2, 14, 14), f"Got {feats.shape}"

    def test_output_shape_T2(self, extractor):
        """(1, 3, 2, 224, 224) → (1, 768, 1, 14, 14)"""
        video = torch.rand(1, 3, 2, 224, 224)
        feats = extractor(video)
        assert feats.shape == (1, 768, 1, 14, 14), f"Got {feats.shape}"

    def test_output_shape_T8(self, extractor):
        """(1, 3, 8, 224, 224) → (1, 768, 4, 14, 14)"""
        video = torch.rand(1, 3, 8, 224, 224)
        feats = extractor(video)
        assert feats.shape == (1, 768, 4, 14, 14), f"Got {feats.shape}"

    def test_output_shape_batch2(self, extractor):
        """Batch size > 1 must work."""
        video = torch.rand(2, 3, 4, 224, 224)
        feats = extractor(video)
        assert feats.shape == (2, 768, 2, 14, 14), f"Got {feats.shape}"
