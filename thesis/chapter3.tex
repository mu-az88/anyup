...
evaluate the proposed AnyUp3D framework. AnyUp addresses the low-resolution
...
to video, AnyUp processes each frame independently, ignoring inter-frame correlations
and producing flickering artifacts and unstable object boundaries. AnyUp3D extends
AnyUp into the temporal domain by integrating two novel modules into its upsampling
...
backbone-agnostic nature of the original AnyUp design.
...
\begin{itemize}
    ...
    \item The system must support arbitrary upsampling scales, preserving the resolution-agnostic behavior of the original AnyUp model.
    \item The system must demonstrably reduce temporal inconsistency artifacts (flickering, jittering object boundaries) compared to the frame-by-frame AnyUp baseline, as measured by the Temporal Consistency (TC) metric and Flow Warping Error on the DAVIS-2017 benchmark.
    \item The system must maintain competitive spatial accuracy, measured by the J\&F score on DAVIS-2017, relative to the original AnyUp baseline.

\end{itemize}

\subsection{Non-Functional Requirements}
\begin{itemize}
    \item \textbf{Backbone Agnosticism:} The temporal modules must integrate into the AnyUp pipeline without making any assumptions about the feature extractor used.
...
When constructing a training clip of $T$ frames from a raw video, two sampling 
...
A video with many frames carries a large amount of temporal redundancy; adjacent 
frames in a high frame-rate clip differ only minimally. If the model were trained 
on densely sampled clips, the temporal consistency objective would be trivially 
satisfied by near-identical consecutive frames and would not generalise to 
realistic temporal gaps. Conversely, sampling with a fixed frame stride on a 
low-frame-rate video would skip a disproportionately large fraction of the visual 
content, distorting the motion dynamics of the clip. By anchoring the sampling interval to $t = 0.5$ seconds (2 frames per second) of wall-clock time, the sampled clips maintain a 
consistent real-world tempo across videos of different frame rates, giving the 
temporal consistency modules a meaningful and uniform learning signal throughout 
training.
...
The spatial upsampling core of AnyUp relies on windowed cross-attention, in which 
...
The AnyUp3D architecture is designed to perform spatiotemporal feature upsampling by processing two distinct input streams: a high-resolution guidance signal and a low-resolution latent feature map. The primary input consists of a \textbf{high-resolution (HR) RGB video} $I \in \mathbb{R}^{B \times 3 \times T \times H \times W}$. This video provides the fine-grained structural and temporal context necessary for high-fidelity upsampling and is processed through a 3D image encoder followed by a \textbf{3D Rotary Positional Encoding (RoPE3D)} module. The RoPE3D operation injects relative spatiotemporal positioning into the flattened video tokens by applying a rotation-based transformation to the query and key representations, ensuring the model remains sensitive to the relative distances between pixels and frames across the temporal dimension.

The second input is a \textbf{low-resolution (LR) feature map} $p \in \mathbb{R}^{B \times C \times T \times h \times w}$. Rather than utilizing static image encoders, this volume is extracted from a frozen, pre-trained \textbf{video processing backbone} such as \textbf{VideoMAE} or \textbf{Video Swin Transformer}. These backbones are uniquely suited to produce feature maps that encapsulate deep semantic information across both spatial and temporal dimensions. Within the AnyUp3D pipeline, these features are passed through a \textbf{Feature Unification (LFU3D)} module, which employs a learned Gaussian derivative basis to map arbitrary input channel dimensions to a unified internal representation ($qk\_dim$). While the HR video generates the Queries ($Q$) and a portion of the Keys ($K_{img}$), the LR feature map serves both as a source for the key-feature encoding ($K_{feat}$) and directly as the Value ($V$) component for the subsequent cross-attention mechanism. All backbone weights are kept frozen throughout training to preserve the backbone-agnostic property of the system.
...
The final stage of the AnyUp3D pipeline performs the localized fusion of high-resolution structural cues and low-resolution semantic features. This is achieved through a windowed spatiotemporal cross-attention mechanism that ensures both spatial detail and temporal consistency in the upsampled output.
...
Notably, the implementation ensures that if $T=1$ and $w_t=0$, the 3D temporal logic collapses, making the system mathematically equivalent to the 2D AnyUp baseline. This maintains backward compatibility with image-based backbones while enabling state-of-the-art spatiotemporal upsampling for video-based encoders.

\section{Dataset Description}
This section describes the datasets utilized to develop and evaluate the performance of the proposed AnyUp3D system. To ensure the model achieves both spatiotemporal robustness and high-fidelity reconstruction, a dual-dataset strategy was adopted: the \textbf{UCF-101} dataset was used for large-scale training, while the \textbf{DAVIS-2017} benchmark served as the primary evaluation set for validating precision and consistency.
...
\begin{itemize}
    ...
    \item \textbf{Relevance to This Work:} While UCF-101 provides the breadth required for training, DAVIS-2017 provides the precision required for rigorous evaluation. Its dense per-frame annotations allow for the precise measurement of:
    \begin{enumerate}
        \item \textbf{Spatial Fidelity:} Evaluated through region similarity ($\mathcal{J}$ score) and contour accuracy ($\mathcal{F}$ score).
        \item \textbf{Temporal Consistency:} Validating the stability of upsampled features across consecutive frames to ensure the absence of flickering or artifacts.
        \item \textbf{Fair Comparison:} As the standard benchmark for related architectures like Cutie and Video Swin Transformer, it enables a direct and transparent performance comparison.
    \end{enumerate}
\end{itemize}

\section{Implementation Details}

The proposed system is implemented entirely in Python, using PyTorch as the deep learning framework. The model is designed to operate on five-dimensional tensors of shape $(B, C, T, H, W)$, representing a batch of $B$ video clips each containing $T$ frames of spatial resolution $H \times W$ with $C$ channels. The overall architecture, referred to as AnyUp3D, extends the pretrained 2D AnyUp model \cite{anyup} into the spatiotemporal domain by introducing a temporal axis across all major computational components, including convolutional layers, positional encodings, and the cross-attention mechanism. A frozen VideoMAE encoder \cite{videomae}, based on a Vision Transformer (ViT-B) pretrained on video data, serves as the teacher model, providing dense spatiotemporal feature targets of shape $(B, 768, T', H', W')$ that supervise the training of AnyUp3D. All experiments are conducted on an NVIDIA T4 GPU in a Google Colab environment, with model checkpoints persisted to Google Drive.
...
\noindent This design decouples the key sequence used for computing attention scores from the value sequence used for aggregation, which is necessary because AnyUp uses the low-resolution feature map as both key and value but allows them to be processed differently.
...
\noindent The primary reconstruction term $\mathcal{L}_{\mathrm{cos\text{-}mse}}$ is the \texttt{Cosine\_MSE} criterion from the original AnyUp codebase, applied to the predicted and teacher feature volumes by flattening the temporal and spatial axes before computing the per-pixel cosine similarity and mean squared error. This loss is backbone-agnostic, operating on normalised feature vectors regardless of their absolute scale.
...
The self-consistency term $\mathcal{L}_{\mathrm{aug}}$ encourages robustness to small perturbations in the guidance signal. Gaussian noise ($\sigma = 0.05$) is added specifically to \texttt{crop\_low} (the low-resolution guidance frames), from which a perturbed set of teacher guidance features is then extracted and downsampled to $7 \times 7$. AnyUp3D is run with this noisy guidance alongside the unchanged high-resolution crop \texttt{crop\_high}, and its output is compared against the clean prediction (held fixed via \texttt{detach}). This design choice targets the guidance path specifically, since it is the low-resolution features---not the high-resolution RGB signal---that are subject to variation across different backbones and encoder configurations.

The temporal consistency term $\mathcal{L}_{\mathrm{temp}}$ penalises frame-to-frame feature differences, modulated by a motion-aware gate derived from the low-resolution guidance frames:
\begin{equation}
\mathcal{L}_{\mathrm{temp}} = \frac{1}{T_t - 1}\sum_{t=1}^{T_t-1} \mathcal{G}_t \odot \|\hat{f}_t - \hat{f}_{t-1}\|_2^2
\end{equation}
where $T_t = T / t_{\text{tubelet}}$ is the number of temporal tokens, and $\mathcal{G}_t$ is the RGB-difference gate:
\begin{equation}
\mathcal{G}_t = 1 - \frac{\Delta_t}{\max_{t'} \Delta_{t'} + \epsilon}, \quad \Delta_t = \frac{1}{C}\sum_c \left| I^{\mathrm{low}}_{t,c} - I^{\mathrm{low}}_{t-1,c} \right|
\end{equation}
$\Delta_t$ is the mean absolute per-channel pixel difference of adjacent low-resolution guidance frames, normalised per-sample. The gate $\mathcal{G}_t \in [0, 1]$ is low where genuine motion is high, which reduces the penalty in regions of fast motion and focuses temporal smoothness enforcement on regions where frame-to-frame change is not justified by the content. This prevents the loss from penalising legitimate fast motion while still suppressing flickering artifacts in static or slow-moving regions. The term is only active when $T_t > 1$. Its effective weight $\lambda_{\mathrm{temp}}(s)$ is linearly ramped from zero to its final value over the first 5{,}000 steps of training, giving the model time to stabilise spatial quality before temporal smoothness is enforced. Loss weights are $(\lambda_{\mathrm{rec}},\,\lambda_{\mathrm{inp}},\,\lambda_{\mathrm{aug}},\,\lambda_{\mathrm{temp}}) = (1.0,\,0.1,\,0.1,\,0.2)$.
...
\begin{itemize}
    ...
    of temporal modules preserves the spatial upsampling quality of the original AnyUp
    ...
    AnyUp baseline, quantified by the mean cosine similarity between adjacent
