...
The RGB-difference gate in the temporal consistency loss excludes frame pairs whose pixel-level difference exceeds a fixed threshold.
...
Beyond the quantitative gains, a meaningful practical contribution is that AnyUp3D treats video as a first-class input.
The original AnyUp must be applied frame-by-frame when used in a video pipeline, producing flickering artifacts that require external post-processing to suppress.
AnyUp3D accepts a video tensor directly and handles temporal coherence internally, making it considerably easier to integrate into downstream video understanding systems without any additional stabilisation stages.
This video-native design is itself a contribution: the model is strictly more capable than its predecessor in the video domain, independent of the specific numerical improvements reported here.
